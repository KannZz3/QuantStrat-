...
*   **框架与流程（严格基于代码文件设计）**：
    *   **02_vol_sorted_technical_anomaly.ipynb**：读取日频面板数据，计算各品种信号日 $s$ 的 60 日滚动历史波动率 `rv_60_signal`；横截面每日分为 `Low / Mid / High` 三个波动率桶（要求每日有效标的不少于 9 个）；以 $s$ 日收盘价计算四类技术指标（MA10, MA20, MA60, MA120）的 `long_flat` 和 `long_short` 仓位信号；执行端采用 $t = s + 1$ 的**延迟交易机制**，以 $t$ 日 Forward 1D 收益率 `fwd_ret_1d`（$t$ 至 $t+1$ 日收盘收益）计算等权策略日收益，并扣除 **5 bps 的单边周转成本**；每日构建 `High - Low` 波动率桶的 HML 策略收益。
...
*   **框架与流程（严格基于代码文件设计）**：
    *   **模型拟合与选择**（`Traditional_Model_Selection_Sugar.ipynb`）：针对 20 个被测品种主力合约日收益率，拟合包含常数均值或 AR(1) 均值方程，结合 GARCH(1,1)、EGARCH(1,1)、EGARCH(2,1)、GJR-GARCH(1,1)、TARCH(1,1) 条件方差方程，以及 Normal、Student-t、Skewed Student-t、GED 误差分布的 19 种参数组合；通过标准化残差 Ljung-Box 自相关检验（$z$ 与 $z^2$ 的 10/20/30 阶滞后）与 ARCH-LM 异方差检验（10阶），在通过诊断且收敛的模型中，以 **BIC 最小化**规则确立最优模型。
    *   **Hong-Lee 广义谱检验**（`hong_lee_20_contracts.ipynb`）：基于广义特征函数残差构建稳健的谱密度函数，在**固定带宽**（主带宽 $p \approx 21$）与**惩罚性数据驱动自适应带宽选择器**下，对最优模型进行频域非线性结构异方差检验，计算 robust M 统计量及 p 值；运行 100 次以最优估计为 DGP 的蒙特卡洛有限样本 Size 仿真，采用 **Wilson Score 方法**计算其 95% 置信区间。
    *   **风险预算与仓位控制**（`06_` 及风控代码）：结合最优估计与厚尾误差分布，计算单日 95% 与 99% 双向 VaR，以 Kupiec POF 回测验证其覆盖率；基于目标波动率计算各品种的仓位缩放参考系数（`Raw Scale`），当品种被谱检验拒绝（`REJECT`）或因自由度估计值 $\nu \le 4$ 导致第四矩理论不收敛时，自动乘以**降权折扣系数（Health Multiplier）**，形成受统计约束的风险调节参考。
*   **数据实证结论与交易指导意义**：
    ...
    *   **第四矩理论收敛受限提示（基于 `10_model_health_summary.csv`）**：检测发现 **AG (沪银)** 和 **CF (棉花)** 等品种的自由度估计值 $\nu \le 4$（AG 为 3.1640，CF 为 3.2287），触发了 `hl_inference_limited_fourth_moment` 警报。
        *   *交易意义*：当 $\nu \le 4$ 时，标准化残差的理论第四矩（峰度）不存在，会导致依赖第四矩收敛的传统渐进方差检验在数理上失效。在构建组合优化模型时，应避免对沪银 and 棉花使用依赖样本高阶矩（如超额峰度）的参数估计方法。
